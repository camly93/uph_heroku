...
def check_rank(sanpham):
    ...
    ten_sp = link_amazon_html.find("span", id="productTitle").text.strip()
    # print(link_ten_sp)
    ...
    for element in re.findall(".*in (.*)", trim_link_amazon):
        if re.match('.*>.*', element) == None:
            ...
            name_group.append(c)
    k = [i.replace(",", "") for i in res]

    class San_pham:
        def __init__(self, id, tensp, rank, group, time):
            self.id = id
            self.tensp = tensp
            self.rank = rank
            self.group = group
    ...
    sanpham_new = San_pham(id, ten_sp, k, name_group, time)
    ...
    for i in range(0, len(sanpham_new.rank)):
        ...
        if len(Sanpham.objects()) == 0:
            ...
            sanpham = Sanpham(id_sanpham=id,ten_sanpham=ten_sp, nhom=[nhom])
            sanpham.save()
        else:
            w = 0
